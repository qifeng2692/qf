...
```typescript
import type { ExtensionAPI } from "@qf/pi-coding-agent";
...
| Package | Purpose |
|---------|---------|
| `@qf/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@qf/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@qf/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@qf/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@qf/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@qf/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@qf/pi-coding-agent";
...
```typescript
import { SessionManager } from "@qf/pi-coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@qf/pi-ai";
...
```typescript
import type { AutocompleteItem } from "@qf/pi-tui";
...
```typescript
import { Box, Text } from "@qf/pi-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@qf/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@qf/pi-ai";
...
```typescript
import { withFileMutationQueue } from "@qf/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@qf/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@qf/pi-coding-agent";
...
```typescript
import { createBashTool } from "@qf/pi-coding-agent";
...
```typescript
import { keyHint } from "@qf/pi-coding-agent";
...
```typescript
import { Text, Component } from "@qf/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@qf/pi-coding-agent";
import { matchesKey } from "@qf/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@qf/pi-coding-agent";
