...
import { readFacetBundleManifest } from "@qf/chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@qf/chord"; import { SlashCommands } from "@qf/pi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		expect(first[0]?.plugin).toEqual({ id: "@qf/test-plugin", version: "1.0.0" });
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@qf/chord"; import { SlashCommands } from "@qf/pi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			...
			writeFile(
				...
				'import { defineFacet } from "@qf/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@qf/chord"; import { SlashCommands } from "@qf/pi-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		...
		expect(manifest.plugin).toEqual({ id: "@qf/pi-example-plugin", version: "1.0.0" });
