# @qf/pi-session-backend-sqlite-node

Node `node:sqlite` Session backend for `@qf/pi-agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@qf/pi-agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@qf/pi-session-backend-sqlite-node";
