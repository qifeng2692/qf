...
`@qf/chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@qf/chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@qf/chord/delta`:

```ts
import { apply, track } from "@qf/chord/delta";
...
`@qf/chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { bundleFacetPackage } from "@qf/chord/bundler";
...
```ts
import { createFacetBundleLoader } from "@qf/chord/node";
