...
Stateful agent with tool execution and event streaming. Built on `@qf/pi-ai`.

## Installation

```bash
npm install @qf/pi-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@qf/pi-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
Transport-neutral facet-service primitives live in `@qf/chord`. The agent core does not export the service runtime.
...
```typescript
declare module "@qf/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@qf/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@qf/pi-agent-core";
