...
const developmentPackages = new Set(["pi-client", "pi-protocol", "pi-server"].map((name) => `@qf/${name}`));
...
for (const name of ["pi-client", "pi-protocol", "pi-server"]) {
  assert.throws(() => import.meta.resolve("@qf/" + name), /Cannot find|cannot find/, name + " must not be installed");
}
