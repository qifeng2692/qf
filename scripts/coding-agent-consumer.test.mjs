...
const codingAgentName = "@qf/pi-coding-agent";
const devPackages = ["pi-client", "pi-protocol", "pi-server"].map((name) => `@qf/${name}`);
...
function createFixture(t, { importServer = false, declareServer = false } = {}) {
	...
	const packages = [codingAgentName, "@qf/chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				...
				dependencies: {
					"@qf/chord": "1.0.0",
					...(declareServer ? { "@qf/pi-server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@qf/pi-server";' : ""}
import { marker } from "@qf/chord";
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@qf/pi-server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@qf/pi-server", version: "1.0.0" }));
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@qf\/pi-server'/);
});
